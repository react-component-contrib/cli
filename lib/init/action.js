...
class Creator {
  ...
  rewriteTemplate() {
    ...
    const README_PATH = path.join(this.context, 'README.md');
    const CHANGELOG_PATH = path.join(this.context, 'CHANGELOG.md');
    const README = fs.readFileSync(README_PATH, 'utf8');
    const CHANGELOG = fs.readFileSync(CHANGELOG_PATH, 'utf8');
    const replaceComponentNameRegexp = /\[Component Name\]/g;
    fs.writeFileSync(
      README_PATH,
      README.replace(replaceComponentNameRegexp, name),
    );
    fs.writeFileSync(
      CHANGELOG_PATH,
      CHANGELOG.replace(replaceComponentNameRegexp, name),
    );
    ...
    componentPackage.homepage = `https://github.com/${organization.substr(1)}/${name}`;
